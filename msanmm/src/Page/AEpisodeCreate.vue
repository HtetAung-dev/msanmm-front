                ...
                <v-form v-model="valid" @submit.prevent="createChapter">
                    <v-container>
                        <v-row>
                            ...
                            <v-col cols="12" md="8">
                                <v-text-field outlined dense rows="15" class="scrollable-textarea" :rules="audioUrlRules"
                                    label="Audio Link ထည့်သွင်းရန်" v-model="audioUrl" variant="solo-filled"></v-text-field>
...
export default {
        ...
        return {
            ...
            bodyRules: [
                v => {
                    ...
                    return "Audio Url link အားထည့်သွင်းရန်လိုအပ်သည်!"
                }
            ]
        }
    },
    methods: {
        ...
        async createChapter() {
            ...
            // price: this.price
            try {
                const res = await axios.post("audios/chapters/create/", formData, { headers: { 'Content-Type': 'multipart/form-data ', 'Authorization': `Bearer ${this.$store.state.token}` } });
                console.log("chapter created", res)
                this.$router.push({ name: 'AudioDetail', params: { id: this.$route.params.id } })
            } catch (error) {
